...
def main():
    ...
    subprocess.run(["python", "01_initial_analysis_plots_all_features.py", user_path])

    print("Selecting features with LSTM...")
    subprocess.run(["python", "02_selected_features_LSTM", user_path])

    print("Running utility curve polynomial fit...")
    subprocess.run(["python", "03_utility_function_calculation"])

    print("Running DE + DQN curve fitting...")
    subprocess.run(["python", "04_DEA_DRL-curve_fitness.py"])
